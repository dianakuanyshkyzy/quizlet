...
export default function TestPageClient({
  ...
  const handleSubmitWritten = (word: Word, value: string) => {
    const correct =
      value.trim().toLowerCase() === word.translation.trim().toLowerCase();
    if (correct) setScore((s) => s + 1);
    else
      setWrongList((arr) => [
        ...arr,
        { word, correct: word.translation, given: value },
      ]);
    ...
    if (correct) setScore((s) => s + 1);
      ...
      setWrongList((arr) => [
        ...arr,
        {
          ...
          correct: (current as any).word.translation,
          given: choice,
        },
      ]);
  ...
  useEffect(() => {
    if (current?.type === "matching") {
      setMatchRightOrder(
        shuffle((current as any).pairs.map((p: Word) => p.translation))
      );
  ...
  const submitMatching = () => {
    ...
    pairs.forEach((p) => {
      ...
      if (
        given &&
        given.trim().toLowerCase() === p.translation.trim().toLowerCase()
      ) {
        correctCount++;
      } else {
        setWrongList((arr) => [
          ...arr,
          { word: p, correct: p.translation, given: matchPairsGiven[p.term] },
        ]);
      }
    });
              ...
              {current.type === "matching" && (
                <div className="w-full max-w-2xl">
                  ...
                  <p className="text-sm text-gray-600 mb-4">
                    Click a term on the left, then its translation below.
                  </p>
